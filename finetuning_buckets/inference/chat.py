class Chat:
    ...
    def __init__(
        self, model, prompt_style, tokenizer, max_length=2048, init_conversation=None, init_system_prompt=None
    ):
        ...
        if prompt_style == "llama2":
            ...
            from finetuning_buckets.models.model_families.llama2_base import (
                LlamaStringConverter,
                base_stopping_criteria,
            ...
            from finetuning_buckets.models.model_families.gemma_base import (
                GemmaStringConverter,
                base_stopping_criteria,
                default_system_prompt,
            )
        ...
        if init_conversation is not None:
            ...
            if init_conversation[-1]["role"] == "user":
                raise ValueError(
                    "the last message of init_conversation should be assistant message or system prompt, not user message"
                )
    ...
    def generate_one_shot_in_batch(
        self,
        inputs,
        accelerator,
        max_new_tokens=1024,
        ...
        **kwargs,
        ...
        for item in inputs:
            if isinstance(item, dict) or isinstance(item, list):
                ...
                item_processed = self.init_conversation() + [
                    {"role": "user", "content": input},
                    {"role": "assistant", "content": ""},
                ]
            else:
    ...
    def validate_conversation(self, conversation=None):
        ...
        while pt < len(convs):
            if convs[pt]["role"] != "user":
                raise ValueError(
                    f"the message should be user - assistant alternation, but the {pt}th message is {convs[pt]['role']}"
                )
            pt += 1
            if pt >= len(convs):
                break
    ...
    def prepare_model_input(self, conversation=None, max_new_tokens=512):
        ...
        tokens_input = self.tokenizer.encode(
            string_input, return_tensors="pt", max_length=self.max_length - max_new_tokens, truncation=True
        )
