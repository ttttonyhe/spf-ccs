...
class ConstrainedSFTTrainer(Trainer):
    ...
    Args:
        ...
        use_gradient_projection (`bool`): Whether to use gradient projection to remove utility gradient components in safety gradient direction.
        safety_dataset (`Optional[Dataset]`): The safety dataset used for computing safety gradients in gradient projection.
        safety_data_collator (`Optional[DataCollator]`): The data collator to use for the safety dataset.
    """
    ...
    def __init__(
        ...
        safety_dataset: Optional[Dataset] = None,
        safety_data_collator: Optional[DataCollator] = None,
    ):
        ...
        self.safety_data_collator = safety_data_collator
        ...
        if self.use_gradient_projection:
            if safety_dataset is None:
                raise ValueError(
                    "Trainer requires a safety dataset, since `use_gradient_projection` is enabled."
                )
            if safety_data_collator is None:
                raise ValueError(
                    "Trainer requires a safety data collator, since `use_gradient_projection` is enabled."
                )
        ...
        with PartialState().local_main_process_first():
            ...
            if safety_dataset is not None:
                safety_dataset = self._prepare_dataset(
                    safety_dataset,
                    tokenizer,
                    packing,
                    dataset_text_field,
    ...
    def get_safety_dataloader(self) -> DataLoader:
        """
        Returns the safety [`~torch.utils.data.DataLoader`].
        """
        if self.safety_dataset is None:
            raise ValueError("Trainer: gradient projection requires a safety_dataset.")
        
        dataloader_params = {
            "batch_size": 1,
            "collate_fn": self.safety_data_collator, 
            "num_workers": self.args.dataloader_num_workers,
            "pin_memory": self.args.dataloader_pin_memory,
            "persistent_workers": self.args.dataloader_persistent_workers,
            "shuffle": True,
        ...
        return self.accelerator.prepare(DataLoader(self.safety_dataset, **dataloader_params))
    ...
    def _inner_training_loop(
        ...
        if self.use_gradient_projection:
            safety_dataloader = self.get_safety_dataloader()
        ...
        if self.is_fsdp_xla_v2_enabled:
            ...
            if self.use_gradient_projection:
                safety_dataloader = tpu_spmd_dataloader(safety_dataloader)
        ...
        if self.use_gradient_projection:
            self.callback_handler.safety_dataloader = safety_dataloader
        ...
        if self.use_gradient_projection:
            safety_data_iterator = iter(safety_dataloader)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.use_gradient_projection:
                    try:
                        safety_inputs = next(safety_data_iterator)
                    except StopIteration:
                        safety_data_iterator = iter(safety_dataloader)
                        safety_inputs = next(safety_data_iterator)
                ...
                with self.accelerator.accumulate(model):
                    if self.use_anchor:
                        anchor_inputs_arg = anchor_inputs
                    else:
                        anchor_inputs_arg = None
                    
                    if self.use_gradient_projection:
                        safety_inputs_arg = safety_inputs
                    else:
                        safety_inputs_arg = None
                    
                    tr_loss_step = self.training_step(model, inputs, anchor_inputs_arg, safety_inputs_arg)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], anchor_inputs: Dict[str, Union[torch.Tensor, Any]], safety_inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        """
        ...
        Args:
            ...
            safety_inputs (`Dict[str, Union[torch.Tensor, Any]]`): the safety inputs for gradient projection
        ...
        if self.use_gradient_projection:
            logger.info("Using gradient projection")
            if self.use_apex:
            ...
            for name, param in model.named_parameters():
                if param.grad is not None:
                    utility_grads[name] = param.grad.clone()
            
            model.zero_grad()
            
            with self.compute_loss_context_manager():
                safety_loss = self.compute_loss(model, safety_inputs, None)
            ...
            for name, param in model.named_parameters():
                if param.grad is not None and name in utility_grads:
                    safety_grad = param.grad
                    utility_grad = utility_grads[name]
                    
                    safety_grad_flat = safety_grad.view(-1)
                    utility_grad_flat = utility_grad.view(-1)
                    
                    dot_product = torch.dot(utility_grad_flat, safety_grad_flat)
                    safety_grad_norm_sq = torch.dot(safety_grad_flat, safety_grad_flat)
                    
                    logger.info(f"dot_product [{dot_product}]")
                    
                    if safety_grad_norm_sq > 1e-8 and dot_product < 0:
                        projection = (dot_product / safety_grad_norm_sq) * safety_grad
                        projected_grad = utility_grad - projection
                        param.grad.copy_(projected_grad)
                    else:
                        param.grad.copy_(utility_grad)
        else:
            if self.use_apex:
                with amp.scale_loss(loss, self.optimizer) as scaled_loss:
                    scaled_loss.backward()
